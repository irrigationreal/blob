...
| **Importers**: `blob import compose|procfile|fly|nextjs|netlify|render|vercel|nix|helm` translate third-party manifests to blob.yaml; `blob deploy --from <kind> <path>` does both in one shot | shipped  |
...
blob import <compose|procfile|fly|nextjs|netlify|render|vercel|nix|helm> <path>
blob login --endpoint URL [--token T]
blob deploy [--name N] [--port P] [--domain D] [--image IMG] [--env ENV] [--cpu C] [--memory M] [--replicas N]
blob deploy --from <compose|procfile|fly|nextjs|netlify|render|vercel|nix|helm> <path>
blob list
blob status <app>
blob logs <app> [-n 200]

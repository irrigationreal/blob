...
| **Importers**: `blob import compose|procfile|fly|nextjs|netlify|render|vercel|nix` translate third-party manifests to blob.yaml; `blob deploy --from <kind> <path>` does both in one shot | shipped  |
...
- Importers beyond compose/procfile/fly/nextjs/netlify/render/vercel/nix: Helm
...
blob import <compose|procfile|fly|nextjs|netlify|render|vercel|nix> <path>
blob login --endpoint URL [--token T]
blob deploy [--name N] [--port P] [--domain D] [--image IMG] [--env ENV] [--cpu C] [--memory M] [--replicas N]
blob deploy --from <compose|procfile|fly|nextjs|netlify|render|vercel|nix> <path>
blob list
blob status <app>
blob logs <app> [-n 200]
